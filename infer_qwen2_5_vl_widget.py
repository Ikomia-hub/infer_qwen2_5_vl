...
from infer_qwen2_5_vl.infer_qwen2_5_vl_process import InferQwen25VlParam
...
# --------------------
class InferQwen25VlWidget(core.CWorkflowTaskWidget):

    def __init__(self, param, parent):
        core.CWorkflowTaskWidget.__init__(self, parent)

        if param is None:
            self.parameters = InferQwen25VlParam()
        else:
...
# --------------------
class InferQwen25VlWidgetFactory(dataprocess.CWidgetFactory):

    def __init__(self):
        dataprocess.CWidgetFactory.__init__(self)
        # Set the algorithm name attribute -> it must be the same as the one declared in the algorithm factory class
        self.name = "infer_qwen2_5_vl"

    def create(self, param):
        # Create widget object
        return InferQwen25VlWidget(param, None)
